...
df_data = pd.read_excel(os.path.join(root_dir,"Covid_data_v2.xlsx"),sheet_name='Extra')
x_scale = [0, 12, 24, 36, 48]
extra_space = 2
vax_window = 'any_vax_3m_3m'
vax_date = 'Vaccine_Date_3m'
...
#───────Pandemic era───────
df_data['Pandemic_Era'] = (pd.to_datetime(df_data['RT.End.Date']) >= '2020-12-01').map({True: 'Yes', False: 'No'})
df_data = df_data[df_data['Pandemic_Era'] =='Yes'].reset_index(drop=True)
df_data = utils.censoring(df_data, time_col,event_col,48)

cols = ['MRN',vax_window,'IO',time_col,event_col]
df_out = df_data[cols]
df_out['Time']  = df_out[time_col] 
df_out['Event']  = df_out[event_col]

df_out['TX_EndDate'] = df_data['RT.End.Date']
df_out['Event_Date'] = df_data[vax_date]
